...
import { FilterBlurBackgroundRenderer } from '../../rendering';
 ...
 */
export class StreamBlurBackgroundStrategy
  implements VideoProcessorBackgroundStrategy
{
  ...
  /** ブラー背景レンダラー */
  private renderer: FilterBlurBackgroundRenderer | null = null;
  /** 処理用のオフスクリーンCanvas */
  ...
  /**
   * StreamBlurBackgroundStrategy のコンストラクタ
  ...
  async initialize(): Promise<void> {
    if (this.isInitialized) {
      throw new Error('StreamBlurBackgroundStrategy is already initialized');
    }
  ...
  /**
   * MediaStreamTrack を処理してブラー背景を適用した新しいトラックを生成する
   * @param track - 処理する元のビデオトラック
   * @returns ブラー背景が適用された MediaStreamTrackGenerator
  ...
  private getProcessedMediaStreamTrack(
    ...
    // ブラー背景レンダラーの初期化
    this.renderer = new FilterBlurBackgroundRenderer(
      this.processCanvas,
      ctx,
      width,
      height,
    ...
    processor.readable
      ...
      .catch((e: Error) => {
        console.error(
          'StreamBlurBackgroundStrategy processing error:',
          e.message
        );
      });
  ...
  async createProcessedStream({
    ...
    if (!this.isInitialized) {
      throw new Error('StreamBlurBackgroundStrategy is not initialized');
    }
